...
def calculate_bollinger_bands(
    ...
    df["BOLLINGER_UPPER"] = middle + standard_deviations * deviation
    df["BOLLINGER_LOWER"] = middle - standard_deviations * deviation
    width = df["BOLLINGER_UPPER"] - df["BOLLINGER_LOWER"]
    df["BOLLINGER_POSITION"] = (
        (df["Close"] - df["BOLLINGER_LOWER"]) / width.replace(0, np.nan)
    )
    return df
...
def get_technical_analysis(ticker: str, df: pd.DataFrame | None = None) -> dict:
    df = get_stock_data(ticker) if df is None else df.copy()
    ...
    def change(period: int) -> float | None:
        if len(df) <= period or float(close.iloc[-period - 1]) == 0:
            return None
        return float(close.iloc[-1] / close.iloc[-period - 1] - 1)

    def slope(column: str, period: int = 5) -> float | None:
        series = df[column].dropna()
        if len(series) <= period:
            return None
        return float(series.iloc[-1] - series.iloc[-period - 1])

    recent_high = float(previous_20["High"].max()) if not previous_20.empty else None
    recent_low = float(previous_20["Low"].min()) if not previous_20.empty else None
    ...
    return {
        ...
        "derived_metrics": {
            ...
            "ma5_slope_5d": slope("MA5"),
            "ma20_slope_5d": slope("MA20"),
            "ma60_slope_5d": slope("MA60"),
            "distance_from_ma20": close_value / ma20 - 1 if ma20 else None,
            "atr_pct": atr / close_value if close_value else None,
            "volatility_20d": float(close.pct_change().tail(20).std()),
            "macd_hist_change_5d": slope("MACD_HIST"),
            "breakout_20d": recent_high is not None and close_value > recent_high,
            "breakdown_20d": recent_low is not None and close_value < recent_low,
        },
        "data_points": len(df),
    }
